...
- Preserve the controller's read-only inline Compose config and short file-based Node command. Never place the complete controller source back into one command or environment argument; Linux rejects a single argument at this size before Node can start.
