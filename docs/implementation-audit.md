...
- Existing bounded audit persistence now captures sanitized decision fields, receipt-time mode/MASTER/exact-direction AUTO state, and allowlisted wallet/balance/reserve/cap details for dashboard activity. Received, temporary handoff-error, and terminal rows merge by bounded ntfy event ID; retries do not create duplicate activity cards, sparse legacy rows cannot erase richer terminal details, unknown generic rows cannot downgrade a specific result, replay noise cannot replace a terminal result, and a receive-only entry older than ten minutes is displayed as interrupted without mutating durable state.
- A successful HTTP status from n8n is not blindly trusted: the body must be a JSON object with a recognized terminal decision, and any durable trade record for that event remains authoritative over a contradictory response. A claimed live-trade outcome without a matching durable controller record is rejected. For malformed, non-object, processing-only, or unknown successful responses, an existing record supplies the idempotent outcome; without one, the controller records a local terminal `ABORTED` result and marks the event seen so a later control change cannot execute that old alert. An unexpectedly `SUBMITTING`/`PENDING` recovered record first persists the safety lock and MASTER off, then becomes `UNCERTAIN`. Network and non-success HTTP failures retain the existing safe reconnect/replay path.
- Recent Alert Activity displays at most ten unique results (three before its nested expander), keeps its latest summary visible while the panel is collapsed, bounds and escapes untrusted text, rejects malformed numeric/mint/signature detail, does not render raw payloads or topic names, and links only validated Base58-shaped signatures.
- Recent Trades displays three durable live-execution records before an expander and at most ten in the dashboard while preserving the existing 500-record durable collection.
...
Each wallet selector, AUTO checkbox, mode control, and confirmation input exists only once in the document. Responsive presentation therefore cannot submit a hidden duplicate value or diverge from the desktop form. Within each mobile coin card, the wallet selector stays grouped with its assignment status and each AUTO direction stays grouped with its own missing-target note. Exact mints, targets, topics, wallet assignments, trade states, and signatures remain visible. Alert activity uses native keyboard-accessible nested disclosure controls, becomes a one-column card at phone widths, and deliberately keeps its latest outcome summary visible while unrelated panel summary metadata is hidden for space. Login, one-time recovery, Gateway diagnostics, readiness, wallet, RPC, safety, and safety-lock forms receive the same mobile overflow, focus, keyboard, and touch-target safeguards.
